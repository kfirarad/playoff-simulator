...
export default function MatchResult({ match, homeTeam, awayTeam }: MatchResultProps) {
  ...
  const handleScoreChange = (team: 'home' | 'away', value: string) => {
    ...
    if (value === "" || isNaN(numValue)) {
      team === 'home' ? setHomeGoals(null) : setAwayGoals(null);
    } else {
      const goals = Math.max(0, Math.min(99, numValue)); // Limit between 0-99
      team === 'home' ? setHomeGoals(goals) : setAwayGoals(goals);
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div className="grid grid-cols-[1fr_80px_1fr] w-full gap-2 items-center">
          ...
          <div className="text-right flex justify-end items-center gap-2">
            ...
            <span className="bg-secondary text-xs rounded-md px-1.5 py-0.5">
              {homeTeam.shortName}
            </span>
          </div>
        ...
        <div className="ml-4">
          {isEditing ? (
            ...
            <Button
              ...
              className="h-8"
            >
              {match.played ? "Change" : "Enter Score"}
            </Button>
          )}
        </div>
      </div>
